class Population {
  ...
  Ball[] population;         // Array to hold the current population
  ...
  void reproduction() {
    ...
    for (int i = 0; i < population.length; i++) {
      ...
      PVector position = new PVector(width/2,height+20);
      population[i] = new Ball(position, child,population.length);
    }
    generations++;
  }
